const MOCK_DATA = {
    "top-news": {
        ...
        "articles": [
            {
                title: "【テストデータ】日銀、マイナス金利解除を決定 17年ぶり利上げ",
                source: "経済ニュース",
                time: "2時間前",
            ...
            {
                title: "【テストデータ】次世代AIモデル「GPT-5」が今夏にもリリースか",
                source: "テクノロジー通信",
                time: "4時間前",
                link: "https://yahoo.co.jp",
                image: "file:///C:/Users/yhisa/.gemini/antigravity/brain/3ec1b64c-9fb8-4f1b-bf97-7bf3efc89321/major_news_thumbnail_1773630566898.png",
    ...
    "startup": {
        ...
        "articles": [
            {
                title: "【テストデータ】国内ユニコーン企業A社、米国展開へ向けて50億円の資金調達",
                source: "STARTUP HUB",
                time: "1時間前",
                link: "https://yahoo.co.jp",
            ...
            {
                title: "【テストデータ】東大発ディープテック・スタートアップがシードで3億円調達",
    ...
    "university": {
        ...
        "articles": [
            {
                title: "【テストデータ】京都大学、全固体電池の寿命を2倍に延長する新素材を発見",
                source: "サイエンスポータル",
                time: "3時間前",
                link: "https://yahoo.co.jp",
            ...
            {
                title: "【テストデータ】文科省、「地域中核大学」のスタートアップ創出支援事業を公募開始",
                source: "大学ジャーナル",
                time: "5時間前",
                link: "https://yahoo.co.jp",
                image: "file:///C:/Users/yhisa/.gemini/antigravity/brain/3ec1b64c-9fb8-4f1b-bf97-7bf3efc89321/university_news_thumbnail_1773630608606.png",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchNewsData() {
        ...
        try {
            ...
            // カテゴリ（タブとセクション）を動的に生成
            setupDynamicCategories(data);

            // 現在アクティブなモードを維持して再描画
            renderCurrentView();
        } catch (error) {
            console.error("データ取得エラー:", error);
            // モックデータを使用する場合も動的にセットアップ
            setupDynamicCategories(MOCK_DATA);
            renderCurrentView();
        }
    }

    function setupDynamicCategories(data) {
        const tabContainer = document.getElementById('category-tabs');
        const sectionContainer = document.getElementById('news-sections-container');
        tabContainer.innerHTML = '';
        sectionContainer.innerHTML = '';

        Object.keys(data).forEach((categoryId, index) => {
            const categoryName = data[categoryId].name || categoryId;

            // タブボタンの生成
            const btn = document.createElement('button');
            btn.className = `tab-btn ${index === 0 ? 'active' : ''}`;
            btn.setAttribute('data-target', categoryId);
            btn.textContent = categoryName;
            btn.onclick = () => switchTab(categoryId);
            tabContainer.appendChild(btn);

            // セクションの生成
            const section = document.createElement('section');
            section.id = categoryId;
            section.className = `news-section ${index === 0 ? 'active-section' : ''}`;
            section.innerHTML = `<div class="news-grid" id="${categoryId}-container"></div>`;
    ...
    const renderNews = (mode) => {
        ...
        Object.keys(currentData).forEach(categoryId => {
            ...
            const articles = currentData[categoryId].articles || [];
